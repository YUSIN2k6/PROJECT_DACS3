package com.example.CoffeeShop.repository;

import com.example.CoffeeShop.entity.MenuItem;
import com.google.firebase.database.DatabaseReference;
import com.google.firebase.database.FirebaseDatabase;
import org.springframework.stereotype.Repository;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;
import java.util.concurrent.CompletableFuture;

@Repository
public class MenuItemRepository extends FirebaseRepositoryBase {
    private final DatabaseReference database;
    private static final String PATH = "menu_items";

    public MenuItemRepository() {
        this.database = FirebaseDatabase.getInstance().getReference(PATH);
    }

    public CompletableFuture<List<MenuItem>> findAll() {
        return listenForSingleValue(database, snapshot -> {
            List<MenuItem> items = new ArrayList<>();
            for (var child : snapshot.getChildren()) {
                MenuItem item = child.getValue(MenuItem.class);
                if (item != null) {
                    item.setId(child.getKey());
                    items.add(item);
                }
            }
            return items;
        });
    }

    public CompletableFuture<MenuItem> findById(String id) {
        return listenForSingleValue(database.child(id), snapshot -> {
            MenuItem item = snapshot.getValue(MenuItem.class);
            if (item != null) {
                item.setId(snapshot.getKey());
            }
            return item;
        });
    }

    public CompletableFuture<Void> save(String id, MenuItem item) {
        return writeValue(database.child(id), item);
    }

    public CompletableFuture<Void> update(String id, Map<String, Object> updates) {
        return updateValues(database.child(id), updates);
    }

    public CompletableFuture<Void> delete(String id) {
        return removeValue(database.child(id));
    }
}
